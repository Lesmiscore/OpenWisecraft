...
public class ServerInfoFragment extends ServerInfoFragmentBase1 {
	static WeakReference<ServerInfoActivity> instance=new WeakReference(null);
	public static List<ServerStatus> stat=new ArrayList<>();
	public static Map<String,Bitmap> faces=ServerInfoActivity.faces;//new HashMap<>();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		int statOfs=getArguments().getInt("statListOffset", -1);
		...
		keeping = getArguments().getBundle("object");
		...
		hideData   = getArguments().getBoolean("nonDetails", false);
		hidePlayer = getArguments().getBoolean("nonPlayers", false);
		hidePlugins = getArguments().getBoolean("nonPlugins", false);
		hideMods = getArguments().getBoolean("nonMods", false);
		...
		nonUpd = getArguments().getBoolean("nonUpd", false);
		noExport = getArguments().getBoolean("noExport", false);
		...
		if (pref.getBoolean("colorFormattedText", false) & pref.getBoolean("darkBackgroundForServerName", false)) {
			...
			if (Build.VERSION.SDK_INT >= 21) {
				//getWindow().setStatusBarColor(DIRT_DARK);//TODO: Fix for this fragment
			}
			...
			psts.setIndicatorColor(ContextCompat.getColor(getContext(), R.color.mainColor));
			psts.setTextColor(ContextCompat.getColor(getContext(), R.color.mainColor));
			psts.setOnPageChangeListener(new ColorUpdater(ContextCompat.getColor(getContext(), R.color.mainColor), PALE_PRIMARY, tabs, psts));
		}

		int offset=getArguments().getInt("offset", 0);
	...
	public synchronized void update(final ServerPingResult resp) {
		...
		Utils.getToolbar(getActivity()).setSubtitle(localStat.toString());
	...
	public void updateTaskDesc(ServerPingResult resp) {
		if (Build.VERSION.SDK_INT >= 21) {
			...
			getActivity().setTaskDescription(td);//Todo: fix
		}
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		switch (item.getItemId()) {
			case 0://Export this ping result
				View dialogView_=getLayoutInflater(null).inflate(R.layout.server_list_imp_exp, null);
				...
				new AppCompatAlertDialog.Builder(getActivity(), R.style.AppAlertDialog)
						...
						public void onClick(DialogInterface di, int w) {
							...
							new AsyncTask<String,Void,File>(){
								...
								public void onPostExecute(File f) {
									if (f != null) {
										Toast.makeText(getActivity(), getResources().getString(R.string.export_complete).replace("[PATH]", f + ""), Toast.LENGTH_LONG).show();
									} else {
										Toast.makeText(getActivity(), getResources().getString(R.string.export_failed), Toast.LENGTH_LONG).show();
									}
								}
			...
			case 1://See the title for all
				AppCompatAlertDialog.Builder ab=new AppCompatAlertDialog.Builder(getActivity(), R.style.AppAlertDialog);
				LinearLayout ll;
				boolean dark;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		// TODO: Implement this method
		return inflater.inflate(R.layout.server_info_pager,container,false);
	}
	...
	class PCUserFaceAdapter extends PlayerNamesListAdapter {
		...
		public PCUserFaceAdapter() {
			super(getActivity(), R.layout.simple_list_item_with_image, new ArrayList<String>());
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			// TODO: Implement this method
			if (convertView == null) {
				convertView = getLayoutInflater(null).inflate(R.layout.simple_list_item_with_image, parent, false);
			}
	...
	class ModInfoListAdapter extends AppBaseArrayAdapter<Object> {
		...
		public ModInfoListAdapter() {
			super(getActivity(), R.layout.simple_list_item_with_image, new ArrayList<Object>());
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			// TODO: Implement this method
			if (position == 0) {
				return getLayoutInflater(null).inflate(R.layout.void_view, null);
			} else {
				position--;
			}
			View v=getLayoutInflater(null).inflate(R.layout.mod_info_content, parent, false);
	...
	class PlayerNamesListAdapter extends AppBaseArrayAdapter<String> {
		public PlayerNamesListAdapter() {
			super(getActivity(), android.R.layout.simple_list_item_1, new ArrayList<String>());
		}
